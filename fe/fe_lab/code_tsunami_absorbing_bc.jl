...
fn = joinpath(@__DIR__, "assets", "finalmesh1.msh")
...
#2x2 matrix boundary element matrix (S_bound)
function boundaryelementmatrix(mesh, element, kconstant) #defined in \Gamma_{2}==edge of the world #mesh=border vertices
    v1 = mesh.vertices[element[1]]
    v2 = mesh.vertices[element[2]]
    el_size = norm(v1-v2)
...
#assembly function for the boundary matrix
...
p1 = Makie.mesh(vertexarray(sea), cellarray(sea), color=real(u_tilda))
...
Makie.save("finemesh1.png", scene_final)